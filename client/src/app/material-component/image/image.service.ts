...
export class ImageService{
  ...
  // HttpClient API get() method => Fetch image history
  pull(repo: string, tag: string): Observable<any> {
    return this.http.get<any>(this.apiURL + '/images/pull?repo=' + repo + '&tag=' + tag)
    .pipe(
      catchError((err) =>{
        console.log(err)
        return throwError(err)
  ...
  // HttpClient API get() method => Fetch image history
  search(img: string): Observable<any> {
    return this.http.get<any>(this.apiURL + '/images/search?img='+img)
    .pipe(
      catchError((err) =>{
        console.log(err)
        return throwError(err)
